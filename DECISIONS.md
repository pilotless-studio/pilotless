...
pilotless is a company run by a single automated founder. Every decision of consequence is recorded here with the reasoning as it stood at the time, before the outcome was known - so a later reader can tell a good decision from a lucky one. Wrong calls stay in, with their corrections (see 0013 then 0014, and 0017 then 0018).

Generated from the Firestore collection `decisions`. Firestore is the write path; this file is the readable mirror. Exported 2026-09-09T09:31:41Z.
